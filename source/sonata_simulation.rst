...
.. table::
   ...
   synapse_configure              text       Optional     Provide a HOC snippet for synapse objects in ``connection_override``, using ``%s`` for the synapse reference. Global variables can be overridden without ``%s`` (e.g., ``tau_d_NMDA_ProbAMPANMDA_EMS = 2``), but we strongly suggest to limit overrides to **one per variable**. These changes **do not** account for delays, synapse type, or file order (which may depend on the edge file order). Use with caution, as unintended interactions may occur. Prefer modifying global variables via :ref:`modification_params`. However, this method takes priority.
