...
.. table::
   ...
   synapse_configure              text       Optional     Provide a snippet of HOC code to be executed on the synapse objects affected by this ``connection_override``. Use ``%s`` as a placeholder where a reference to the synapse object should be inserted. You can also override global variables without using ``%s``. For example ``tau_d_NMDA_ProbAMPANMDA_EMS = 2``. However, it is strongly recommended to limit overrides to **one per variable**. These overrides currently **do not** account for: delays, synapse type, order in the file (behavior may depend on the edge file order). Use with caution, as unintended interactions may occur.
